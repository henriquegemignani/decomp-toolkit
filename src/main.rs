...
enum SubCommand {
    ...
    Splits(cmd::splits::Args),
    SymbolMatch(cmd::symbol_match::Args),
...
fn main() {
    ...
    result = result.and_then(|_| match args.command {
        ...
        SubCommand::Splits(c_args) => cmd::splits::run(c_args),
        SubCommand::SymbolMatch(c_args) => cmd::symbol_match::run(c_args),
        SubCommand::Symbols(c_args) => cmd::symbols::run(c_args),
